...
post_summary2 =function(tm, 
  ...
  #p01=rr$p; d01=rr$dp
  #p10=rr$p; d10=rr$dp
  #lw01 = sapply(p01, ldp01, tm=tm);
  #lw10 = sapply(p10, ldp10, tm=tm);
  
  if(tm$zz<5){ p01 = 1.; lw01=0; d01=1;} else {
    p01=rr$p; d01=rr$dp
    lw01 = sapply(p01, ldp01, tm=tm);}
  if(tm$uu<5){ p10 = 1.; lw10=0; d10=1;} else {
    p10=rr$p; d10=rr$dp; 
    lw10 = sapply(p10, ldp10, tm=tm);}

  m = outer(p01,p10,function(x,y) x/(x+y))
  w = outer(lw01,lw10, function(x,y) {exp(x+y)})
  d = outer(d01,d10, function(x,y) x*y)
  #sum(w*d)
  
  if(verbose) {  print(sum(w*d))  }
  if(!abs(sum(w*d)-1)<tolerance){ 
    print(sum(w*d))
    print(tm)}
  assertthat::assert_that(abs(sum(w*d)-1)<tolerance)
  mc = m[w*d>cutoff]
  wc = (w*d)[w*d>cutoff]
  ro = order(mc)
  ss = cumsum(wc[ro])
...
get_postsummary = function(tr) {
  post_summary2(tmat(tr,stride=10), nbin=1000, tolerance=1e-2) %>% 
    as.list %>% as_tibble
}
...
data_complete %>% 
  ...
  mutate(interaction = recode(interaction, Hbond_ProA='Hbond', Hbond_ProD='Hbond')) %>%
  ...
  scale_fill_manual(values=c('#fdae6b', #apolar
                             '#e34a33', ##  #aro  red
                             '#253494', #elec dark blue 
                             '#2c7fb8', #hb  mid blue
                             '#41b6c4'  #wb  light blue b1
  ), drop=TRUE) +
...
ggsave("facet_int-marta.pdf", width = 9, height = 6)
...
#polar_palette = c('#253494', '#2c7fb8', '#41b6c4')
aro_palette = brewer.pal(n = 9, "Reds")[1:5]
polar_palette = brewer.pal(n = 9, "YlGnBu")[3:8]
apolar_palette = brewer.pal(n = 9, "Oranges")[2:5]

ggsave("facet_ab-marta.pdf", width = 9, height = 6)
...
pymol_command = function(resID, chain, interac, strength){
  ...
  if(interac=='polar') { 
      p1=colorRamp(polar_palette) } else if(interac=='apolar') {
      p1=colorRamp(apolar_palette) } else {
        p1=colorRamp(aro_palette)
      }
